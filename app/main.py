...
from .schemas import EngineState, FaultRequest, HealthResponse, TelemetryPoint, UAV, UAVCreate
...
@app.post("/uavs", response_model=UAV, status_code=201)
def create_uav(body: UAVCreate):
    if not body.name.strip() or not body.model.strip():
        raise HTTPException(status_code=422, detail="name and model are required")

    base_id = _slugify(body.name)
    uav_id = base_id
    suffix = 2
    while database.uav_exists(uav_id):
        uav_id = f"{base_id}-{suffix}"
        suffix += 1

    return database.create_uav(uav_id, body.name.strip(), body.model.strip())
